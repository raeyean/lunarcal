// 神诞 / 佛诞 — deity birthdays & celestial feasts (lunar calendar)
// Keyed by `${lunarMonth}-${lunarDay}` — leap months ignored (not surfaced).
// Ported from the LunarCal design handoff (lunar-data.js).

import type { DeityKind } from '../constants/colors';

export interface DeityDay {
  /** Full ceremonial name shown on cards. */
  name: string;
  /** Short deity name shown on calendar cells. */
  deity: string;
  /** Tradition kind — drives chip colour. */
  kind: DeityKind;
}
